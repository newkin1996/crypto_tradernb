    ...
    try:
            ...
            try:
                ...
                cursor.execute(sql.SQL(
                    "insert into error_log(symbol, order_action, entry_order_type, exit_order_type, quantity, occured_time, error_description) values (%s, %s, %s, %s, %s, %s, %s)"),
                               [SPOT_SYMBOL, SPOT_SIDE, JSON_ENTRY, JSON_EXIT, SPOT_QUANTITY, error_occured_time,
                                error_occured])
                conn.commit()
        ...
        if sell_leverage_alert == 1 and entry_order_type == "Market":
            try:
                ...
                cursor.execute(sql.SQL(
                    "insert into error_log(occured_time, symbol, order_action, entry_order_type, exit_order_type, quantity, error_description) values (%s, %s, %s, %s, %s, %s, %s)"),
                               [error_occured_time, SPOT_SYMBOL, SPOT_SIDE, JSON_ENTRY, JSON_EXIT, SPOT_QUANTITY,
                                error_occured])
                conn.commit()
    except:
        pass
